...
/**
 * @brief Converts services list teaser images to WebP without resizing.
 *
 * @date 2026-03-22
 * @author Stephane H.
 */
class ServiceTeaserImageProcessor
{
    private const TARGET_RELATIVE_DIR = 'uploads/service-teasers';
    ...
    public function processToWebp(UploadedFile $file, string $projectDir, int $serviceId): string
    {
        ...
        if (!is_dir($uploadDir) && !mkdir($uploadDir, 0755, true) && !is_dir($uploadDir)) {
            ...
            throw new \RuntimeException('Unable to create destination directory for service teaser images.');
        }

        $filename = sprintf('service-teaser-%d-%s.webp', $serviceId, bin2hex(random_bytes(8)));
        ...
        if ($written === false) {
            throw new \RuntimeException('Unable to convert service teaser image to WebP.');
        }
