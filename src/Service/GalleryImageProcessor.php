...
/**
 * @brief Converts gallery images to WebP format without resizing.
 *
 * @date 2026-03-22
 * @author Stephane H.
 */
...
{
    private const TARGET_RELATIVE_DIR = 'uploads/gallery';
    ...
    public function processToWebp(UploadedFile $file, string $projectDir): string
    {
        ...
        if (!is_dir($uploadDir) && !mkdir($uploadDir, 0755, true) && !is_dir($uploadDir)) {
            ...
            throw new \RuntimeException('Unable to create destination directory for gallery photos.');
        }

        $filename = sprintf('gallery-%s.webp', bin2hex(random_bytes(8)));
        ...
        if ($written === false) {
            throw new \RuntimeException('Unable to convert gallery image to WebP.');
        }
