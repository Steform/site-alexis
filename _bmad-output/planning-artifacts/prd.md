...
- **Messages à date bornée :** Alexis peut créer **deux messages différents** qui s’affichent uniquement sur une **période donnée** (date début, date fin). Exemple : « Le garage sera fermé du 12 au 22 février pour cause de congé ». L’emplacement exact à l’écran sera précisé en phase de réalisation ; le back-office doit permettre la gestion (contenu + dates) de ces deux messages.
...
- **Autonomie :** Alexis met à jour horaires et avis (saisie manuelle type Google) sans dépendre du dev ; gestion de deux messages à date bornée (ex. fermeture pour congés).
...
- Alexis peut modifier horaires, avis et 2 messages à date bornée via le back-office.
...
- Site public : pages selon maquette, horaires, avis (saisis manuellement), CTA « Demander un devis » → email, 2 messages à date bornée (ex. fermeture).
- Back-office : compte admin, compte Alexis ; Alexis peut gérer horaires, avis, 2 messages (contenu + dates).
...
- Option : plus de 2 messages ou types d’annonces si besoin.
...
- **Site public :** pages alignées sur la maquette ; horaires dynamiques ; affichage des avis (saisis manuellement) ; affichage conditionnel des 2 messages à date bornée ; formulaire « Demander un devis » avec envoi d’email ; contact visible (téléphone, etc.) ; identité Alexis (nom, photo, texte).
- **Back-office Alexis :** authentification ; écrans de gestion des horaires, des avis et des 2 messages (contenu + dates) ; pas d’accès à la gestion des autres comptes.
